...
class AssetStylesheetLoader implements StylesheetLoader
{
    ...
    {
        try {
            $css = $this->cssProcessor->process($this->contentOf($file));
    ...
    private function contentOf(string $file): string
    {
        $asset = $this->assetRepository->createAsset($file, $this->designParams());
